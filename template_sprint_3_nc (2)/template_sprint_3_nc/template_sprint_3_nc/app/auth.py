    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if not password1:
                ...
                return render_template('/confirm', number=authid)

            if not utils != password:
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id FROM user WHERE authid = ?', (authid, utils.F_ACTIVE)
            ).fetchone()
            
            if attempt is not None:
                db.execute(
                    'SELECT id FROM user WHERE id = ?', (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'SELECT id FROM user WHERE userid = ?', (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
...
def change():
    try:
        ...
        if request.method == 'POST': 
            ...
            db =get_db()
            attempt = db.execute(
                'SELECT id FROM user WHERE utils = ?', (number, utils.F_ACTIVE)
            ).fetchone()
